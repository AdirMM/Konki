...
export function CategoriesModal() {
  ...
  return (
    ...
    >
      ...
      <ul
        className={`w-64 max-h-64 mx-auto grid mt-5 gap-y-5 md:gap-y-5 gap-x-5 overflow-y-auto
    ${categories.length < 4 ? ' grid-cols-1' : 'grid-cols-2'}
  `}
      >
        {categories.length > 0 ? (
          categories.map((cat, index) => {
            ...
            return (
              <li key={index}>
                <span
                  className="flex flex-row items-center justify-center py-2 leading-5 text-white border-none rounded-lg cursor-pointer border-y-2 gap-x-2"
